...
class RaftNode {
    ...
    resetElectionTimer() {
        ...
        const timeout = Math.random() * 4000 + 8000;
    ...
    startHeartbeat() {
        ...
        this.heartbeatInterval = setInterval(() => {
            if (this.state === "LEADER") { this.sendHeartbeat(); }
    ...
    async appendEntry(data) {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    ...
                    const res = await axios.post(`${peer}/raft/append-entries`, {
                        term: this.currentTerm, leaderId: this.id,
                        prevLogIndex, prevLogTerm, entries: [entry], leaderCommit: this.commitIndex
                    }, { timeout: 2000 });
                    if (res.data.success) {
        ...
        const majority = Math.floor((this.peers.length + 1) / 2) + 1;
        if (acks >= majority) { this.commitIndex = entry.index; this.lastApplied = entry.index; return true; }
        return false;
    }
    ...
    handleRequestVote(data) {
        ...
        if (term === this.currentTerm && (this.votedFor === null || this.votedFor === candidateId)) {
            ...
            if (candidateLogOk) { voteGranted = true; this.votedFor = candidateId; this.resetElectionTimer(); }
        }
